...
//  AIChatCourse
//
//  Created by Tung Le on 11/12/2025.
//
import SwiftUI
...
struct CoreInteractor {
    ...
    func markOnboardingCompleted(profileColorHex: String) async throws {
        try await userManager.markOnboardingCompleted(profileColorHex: profileColorHex)
    }
    ...
    func getFeaturedAvatar() async throws -> [AvatarModel] {
        try await avatarManager.getFeaturedAvatar()
    }
    
    func getPopularAvatar() async throws -> [AvatarModel] {
        try await avatarManager.getPopularAvatar()
    }
    
    func getAvatarForCategory(category: CharacterOption) async throws -> [AvatarModel] {
        try await avatarManager.getAvatarForCategory(category: category)
    }
    ...
    func streamChatMessage(chatId: String) -> AsyncThrowingStream<[ChatMessageModel], Error> {
        chatManager.streamChatMessage(chatId: chatId)
    }
