...
class ChatViewModel {
    ...
    func listenForChatMessage() async {
        ...
        do {
            ...
            for try await value in interactor.streamChatMessage(chatId: chatId) {
                chatMessages = value
                    .sortedByKeyPath(keyPath: \.dateCreatedCalculated, ascending: true)
    ...
    func onSendMessagePressed(avatarId: String) {
        ...
        Task {
            do {
                ...
                /// If chat is new, then create a new chat
                if chat == nil {
                ...
                if let avatarDescription = avatar?.characterDescription {
                    ...
                    let systemMessage = AIChatModel(
                        role: .system,
                        content: "You are \(avatarDescription) with the intelligence of an AI. We are having a VERY casual conversation. You are my friend."
                    )
    ...
    func onChatSettingPressed(onDidDeleteChat: @escaping @MainActor () -> Void) {
            ...
            buttons: {
                AnyView(
                    Group {
                        ...
                        Button("Delete Chat", role: .destructive) { [onDidDeleteChat] in
    ...
    func onProfileModalXmarkPressed(chat: ChatModel?) {
