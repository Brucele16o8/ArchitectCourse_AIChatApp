...
fileprivate extension String {
    func clean(maxCharacter: Int) -> String {
        self
            .clipped(maxCharacter: maxCharacter)
...
struct FirebaseAnalyticsService: LogService {
    ...
    func addUserProperties(dict: [String: Any], isHighPriority: Bool) {
        ...
        for (key, value) in dict {
            if let string = String.convertToString(value) {
                let key = key.clean(maxCharacter: 40)
                let string = string.clean(maxCharacter: 100)
    ...
    func trackEvent(event: any LoggableEvent) {
        ...
        /// Fix key/value length limit
        for (key, value) in parameters where key.count > 40 {
            parameters.removeValue(forKey: key)
            
            let newKey = key.clean(maxCharacter: 40)
        ...
        for (key, value) in parameters {
            if let string = value as? String {
                parameters[key] = string.clean(maxCharacter: 100)
            }
        }
        
        parameters.first(upTo: 25)
        
        let name = event.eventName.clean(maxCharacter: 40)
